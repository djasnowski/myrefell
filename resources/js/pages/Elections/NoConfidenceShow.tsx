...
import { AlertTriangle, CheckCircle, Clock, ScrollText, ThumbsDown, ThumbsUp, Users, XCircle } from 'lucide-react';
...
export default function NoConfidenceShow({ vote, user_state }: Props) {
    ...
    const handleVote = (voteForRemoval: boolean) => {
        ...
        setVoting(true);
        router.post(`/no-confidence/${vote.id}/vote`, { vote_for_removal: voteForRemoval }, {
            preserveScroll: true,
    ...
    const getStatusText = (status: string) => {
        switch (status) {
            case 'passed': return 'Removed from Office';
            case 'failed': return 'Retained Position';
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 text-stone-400">
                            <span className="font-pixel text-xs">
                                Against <span className="text-red-300">{vote.target_player.username}</span>
                                {' '}(<span className="capitalize">{vote.target_role}</span>)
                            </span>
                            <span className={`rounded px-2 py-0.5 font-pixel text-[10px] capitalize ${getStatusColor(vote.status)}`}>
                ...
                <div className="mx-auto w-full max-w-4xl">
                        ...
                        <div className="rounded-xl border-2 border-stone-700 bg-stone-800/50 p-4">
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between">
                                    <span className="font-pixel text-[10px] text-stone-500">Target</span>
                                    <span className="font-pixel text-xs text-red-400">{vote.target_player.username}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="font-pixel text-[10px] text-stone-500">Role</span>
                                    <span className="font-pixel text-xs text-stone-300 capitalize">{vote.target_role}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="font-pixel text-[10px] text-stone-500">Location</span>
                                    <span className="font-pixel text-xs text-stone-300">{vote.domain_name}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="font-pixel text-[10px] text-stone-500">Domain</span>
                                    <span className="font-pixel text-xs text-stone-300 capitalize">{vote.domain_type}</span>
                                </div>
                                {vote.voting_ends_at && (
                                    <div className="flex justify-between">
                                ...
                                {vote.initiated_by && (
                                    <div className="flex justify-between">
                                        <span className="font-pixel text-[10px] text-stone-500">Initiated By</span>
                                        <span className="font-pixel text-xs text-stone-300">{vote.initiated_by.username}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="rounded-xl border-2 border-stone-700 bg-stone-800/50 p-4">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <div className="mb-2 flex justify-between font-pixel text-[10px]">
                                        <span className="flex items-center gap-1 text-red-400">
                                            <ThumbsUp className="h-3 w-3" /> For Removal ({vote.votes_for})
                                        </span>
                                        <span className="flex items-center gap-1 text-green-400">
                                            Against ({vote.votes_against}) <ThumbsDown className="h-3 w-3" />
                                        </span>
                                    </div>
                                ...
                                <div className="flex items-center justify-between rounded-lg border border-stone-700 bg-stone-800/30 p-2">
                                    ...
                                    {vote.quorum_met ? (
                                        <span className="flex items-center gap-1 font-pixel text-xs text-green-400">
                                            <CheckCircle className="h-3 w-3" /> Met ({vote.votes_cast}/{vote.quorum_required})
                                        </span>
                                    ) : (
                                        <span className="flex items-center gap-1 font-pixel text-xs text-stone-400">
                                            <XCircle className="h-3 w-3" /> {vote.quorum_required - vote.votes_cast} more needed
                                        </span>
                                    )}
                                </div>
                                ...
                                <div className="flex items-center justify-between rounded-lg border border-stone-700 bg-stone-800/30 p-2">
                                    ...
                                    <span className="font-pixel text-xs text-stone-300">{vote.eligible_voters}</span>
                                </div>

                                {vote.is_open && (
                                    <div className="flex items-center gap-2 rounded-lg border border-yellow-600/50 bg-yellow-900/20 p-2">
                                        <Clock className="h-4 w-4 text-yellow-400" />
                                        <span className="font-pixel text-xs text-yellow-400">Voting Open</span>
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="space-y-3">
                                ...
                                {user_state.is_target && (
                                    <div className="rounded-lg border border-red-600/50 bg-red-900/20 p-2 text-center">
                                        <span className="font-pixel text-xs text-red-400">You are the target of this vote</span>
                                    </div>
                                )}

                                {user_state.is_initiator && (
                                    <div className="rounded-lg border border-yellow-600/50 bg-yellow-900/20 p-2 text-center">
                                        <span className="font-pixel text-xs text-yellow-400">You initiated this vote</span>
                                    </div>
                                )}
                                ...
                                {user_state.has_voted && (
                                    <div className="rounded-lg border border-stone-600 bg-stone-800/50 p-2 text-center">
                                        <span className="font-pixel text-xs text-stone-400">You have already voted</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {vote.reason && (
                        <div className="mt-4 rounded-xl border-2 border-stone-700 bg-stone-800/50 p-4">
                            <h2 className="mb-2 font-pixel text-sm text-stone-300">Reason for Vote</h2>
                    ...
                    {vote.notes && (
                        <div className={`mt-4 rounded-xl border-2 p-4 ${
                            ...
                            <h2 className={`mb-2 font-pixel text-sm ${
                                vote.status === 'passed' ? 'text-red-400' :
                                vote.status === 'failed' ? 'text-green-400' :
                                'text-stone-300'
                            }`}>Outcome</h2>
