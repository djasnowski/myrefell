...
export default function FarmingIndex() {
    const { plots, crop_types, farming_skill, max_plots, gold, master_farmer_bonuses, error } = usePage<PageProps>().props;
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                ...
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <p className="font-pixel text-sm text-stone-400">Grow crops and harvest your yield</p>
                    </div>
                ...
                {master_farmer_bonuses && (
                    <div className="rounded-lg border border-amber-500/50 bg-amber-900/20 p-3">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-pixel text-xs text-amber-300">Master Farmer Bonus Active!</span>
                        </div>
                ...
                <div className="rounded-lg border border-green-700/50 bg-green-900/20 p-3">
                    <div className="mb-1 flex items-center justify-between font-pixel text-xs">
                        ...
                        <span className="text-stone-300">
                            {farming_skill.xp.toLocaleString()} / {farming_skill.xp_to_next.toLocaleString()} XP
                        </span>
                    </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        ...
                        return (
                            ...
                            >
                                <div className="mb-3 flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <StatusIcon className={`h-5 w-5 ${plot.has_withered ? 'text-red-400' : plot.is_ready ? 'text-yellow-400' : 'text-green-400'}`} />
                                        <span className="font-pixel text-sm capitalize text-stone-300">
                                            {plot.crop?.name || 'Empty Plot'}
                                        </span>
                                    </div>
                                    {plot.is_watered && (
                                        <Droplets className="h-4 w-4 text-blue-400" title="Watered" />
                                    )}
                                </div>
                                ...
                                {plot.status !== 'empty' && (
                                    <>
                                        ...
                                        <div className="mb-2 flex items-center gap-2 font-pixel text-[10px]">
                                            ...
                                            <span className="text-stone-400">Quality: {plot.quality}%</span>
                                            <span className="text-stone-500">({plot.times_tended}x tended)</span>
                                        </div>
                                    ...
                                    <div className="mt-3 flex flex-wrap gap-2 border-t border-stone-700 pt-3">
                                        {plot.status === 'empty' && (
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    if (e.target.value) {
                                                        handleAction('plant', plot.id, { crop_type_id: parseInt(e.target.value) });
                                                    }
                                                }}
                                                disabled={loading !== null}
                                            >
                                                <option value="">Select crop to plant...</option>
                                                {crop_types.filter(c => c.can_plant).map((crop) => (
                                                    <option key={crop.id} value={crop.id}>
                                                        {crop.name} ({crop.plant_cost}g, {formatTime(crop.grow_time_minutes)})
                                                    </option>
                                                ))}
                                            </select>
                                        )}

                                        {['planted', 'growing'].includes(plot.status) && !plot.is_watered && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); handleAction('water', plot.id); }}
                                                disabled={loading !== null}
                                        ...
                                        {['planted', 'growing'].includes(plot.status) && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); handleAction('tend', plot.id); }}
                                                disabled={loading !== null}
                                        ...
                                        {plot.is_ready && !plot.has_withered && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); handleAction('harvest', plot.id); }}
                                                disabled={loading !== null}
                                        ...
                                        {(plot.has_withered || plot.status !== 'empty') && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); handleAction('clear', plot.id); }}
                                                disabled={loading !== null}
                    ...
                    {currentPlotCount < max_plots && (
                        ...
                        >
                            ...
                            <span className="font-pixel text-[10px] text-stone-400">{nextPlotCost} gold</span>
                        </button>
                    )}
                </div>
                ...
                {plots.length === 0 && (
                    <div className="flex flex-1 items-center justify-center">
                        <div className="text-center">
                            ...
                            <p className="mb-4 font-pixel text-sm text-stone-500">Purchase your first plot to start farming</p>
                            <button
                                onClick={() => handleAction('buy-plot')}
                            ...
                            {crop_types.map((crop) => (
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <span className="font-pixel text-xs text-stone-200">{crop.name}</span>
                                        <span className="font-pixel text-[10px] text-green-400">Lvl {crop.farming_level_required}</span>
                                    </div>
                                    <div className="mt-1 flex items-center gap-2 font-pixel text-[10px] text-stone-400">
                                        <span><Clock className="inline h-2 w-2" /> {formatTime(crop.grow_time_minutes)}</span>
                                        <span><Coins className="inline h-2 w-2" /> {crop.plant_cost}g</span>
                                        <span><Wheat className="inline h-2 w-2" /> {crop.yield_min}-{crop.yield_max}</span>
                                    </div>
                                    {!crop.can_plant && (
                                        <span className="font-pixel text-[10px] text-red-400">Out of season</span>
                                    )}
                                </div>
                            ))}
                        </div>
