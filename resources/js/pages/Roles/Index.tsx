...
const roleConsequences: Record<string, { duties: string[]; ifVacant: string }> = {
    elder: {
        duties: ['Set village taxes', 'Appoint other roles', 'Settle disputes', 'Manage village funds'],
        ifVacant: 'No taxes collected, no new appointments, village decisions stall.',
    },
    guard_captain: {
        duties: ['Patrol the village', 'Arrest criminals', 'Defend against attacks', 'Train militia'],
    ...
    merchant: {
        duties: ['Buy goods from players', 'Sell supplies', 'Manage market prices', 'Handle trade deals'],
    ...
    master_farmer: {
        duties: ['Oversee village crops', 'Distribute seeds', 'Set harvest schedules', 'Manage livestock'],
...
function RoleModal({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
            ...
            >
                {/* Close button */}
                <button onClick={onClose} className="absolute right-3 top-3 text-stone-400 hover:text-white">
                    <X className="h-5 w-5" />
                </button>
                ...
                <div className="mb-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            <span className={`rounded px-1.5 py-0.5 font-pixel text-[10px] ${tierBadgeColors[role.tier]}`}>
                                Tier {role.tier}
                            </span>
                            {role.is_elected && <span className="font-pixel text-[10px] text-purple-400">Elected Position</span>}
                            <span className={`font-pixel text-[10px] ${role.is_vacant ? 'text-red-400' : 'text-green-400'}`}>
                ...
                <div className="mb-4 rounded-lg bg-stone-900/50 p-3">
                    <div className="mb-2 flex items-center gap-2">
                        ...
                        <span className="font-pixel text-sm text-amber-300">{role.salary}g / day</span>
                    </div>
                    {Object.keys(role.bonuses).length > 0 && (
                        <div className="flex flex-wrap gap-2">
                            {Object.entries(role.bonuses).map(([key, value]) => (
                                <span key={key} className="rounded bg-green-900/50 px-2 py-0.5 font-pixel text-[10px] text-green-300">
                ...
                <div className="mb-4">
                    ...
                    <ul className="space-y-1">
                        {consequences.duties.map((duty, i) => (
                            <li key={i} className="flex items-center gap-2 font-pixel text-xs text-stone-300">
                ...
                {role.is_vacant && (
                    <div className="mb-4 rounded-lg border border-red-600/50 bg-red-900/20 p-3">
                        <div className="flex items-start gap-2">
                            ...
                            <div>
                                <span className="font-pixel text-xs text-red-300">Without a {role.name}:</span>
                                <p className="font-pixel text-[10px] text-red-400/80">{consequences.ifVacant}</p>
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="mb-4 rounded-lg bg-stone-900/50 p-3">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1 text-stone-500">
                                ...
                                <span className="font-pixel text-[10px]">Since {formatDate(role.holder.appointed_at)}</span>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="flex gap-2">
                    {isUserRole && (
                        ...
                        >
                            {resignLoading === userRoleHere!.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <><LogOut className="h-4 w-4" />Resign</>}
                        </button>
                    )}
                    {canClaim && (
                        <button
...
export default function RolesIndex() {
    ...
    const handleResign = (playerRoleId: number) => {
        ...
        router.post(`/roles/${playerRoleId}/resign`, {}, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleClaim = (roleId: number) => {
        ...
        router.post('/roles/claim', { role_id: roleId, location_type, location_id }, {
            preserveScroll: true,
            onSuccess: () => {
                setSelectedRole(null);
                router.reload();
    ...
    // Group roles by tier
    const rolesByTier = roles.reduce((acc, role) => {
        if (!acc[role.tier]) acc[role.tier] = [];
        acc[role.tier].push(role);
        return acc;
    }, {} as Record<number, Role[]>);

    const sortedTiers = Object.keys(rolesByTier).map(Number).sort((a, b) => b - a);
    const tierLabels: Record<number, string> = { 5: 'Rulers', 4: 'Leadership', 3: 'Senior', 2: 'Skilled', 1: 'Workers' };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col overflow-hidden p-4">
                ...
                <div className="mb-4 flex items-center justify-between">
                    <div>
                        <h1 className="font-pixel text-xl text-amber-400">{locationTypeDisplay} Roles</h1>
                ...
                <div className="-mx-1 space-y-4 overflow-y-auto px-1">
                    {sortedTiers.map((tier) => (
                        <div key={tier}>
                            <h2 className="mb-2 font-pixel text-sm text-stone-400">{tierLabels[tier] || `Tier ${tier}`}</h2>
                            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                {rolesByTier[tier].map((role) => {
                                    const Icon = iconMap[role.icon.toLowerCase()] || Crown;
                                    const isUserRole = user_roles_here.some((ur) => ur.role_id === role.id);
                                        ...
                                        >
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center justify-between gap-2">
                                                    <span className="font-pixel text-sm text-amber-300 leading-tight truncate">{role.name}</span>
                                                    <div className="flex items-center gap-2 flex-shrink-0">
                                                        {role.holder && (
                                                            <span className="font-pixel text-xs text-stone-300">
                                                                {role.holder.username}
                                                                <span className="ml-1 text-stone-500">
                                                                    Lv.{role.holder.total_level} {role.holder.title.charAt(0).toUpperCase() + role.holder.title.slice(1)} ({role.holder.social_class.charAt(0).toUpperCase() + role.holder.social_class.slice(1)})
                                                                </span>
                                                            </span>
                                                        )}
                                                        <span className={`h-2.5 w-2.5 rounded-full ${role.is_vacant ? 'bg-red-500' : 'bg-green-500'}`} />
                                                    </div>
                                                </div>
                                                <div className="font-pixel text-xs text-stone-400 leading-snug truncate max-w-[220px] mt-1">{roleShortDesc[role.slug] || role.description.slice(0, 35)}</div>
                                                <div className="font-pixel text-xs text-stone-500 mt-1">{role.salary}g/day</div>
                                            </div>
                                        </button>
                                    );
                                })}
