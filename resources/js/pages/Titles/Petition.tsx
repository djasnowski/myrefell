        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col overflow-auto p-4">
                ...
                <div className="mx-auto w-full max-w-2xl space-y-6">
                    {/* Title Info */}
                    <div
                        className={`rounded-xl border-2 p-6 ${getCategoryBorder(title_type.category)}`}
                    >
                        <div className="flex items-center gap-4">
                            <div
                                className={`flex h-14 w-14 items-center justify-center rounded-lg ${
                                    title_type.category === "royalty"
                                        ? "bg-purple-900/50"
                                        : title_type.category === "landed_nobility"
                                          ? "bg-amber-900/50"
                                          : title_type.category === "minor_nobility"
                                            ? "bg-blue-900/50"
                                            : "bg-stone-800/50"
                                }`}
                            >
                                <Crown
                                    className={`h-8 w-8 ${getCategoryColor(title_type.category)}`}
                                />
                            </div>
                            <div className="flex-1">
                                <h1
                                    className={`font-pixel text-2xl ${getCategoryColor(title_type.category)}`}
                                >
                                    {title_type.name}
                                </h1>
                    ...
                    {meets_requirements && (
                        <div className="rounded-xl border-2 border-stone-700 bg-stone-800/50 p-4">
                                ...
                                <div className="mb-4">
                                    ...
                                    >
                                        <div className="flex items-center gap-2">
                                            <Coins
                                                className={`h-5 w-5 ${isPurchase ? "text-amber-400" : "text-stone-500"}`}
                                            />
                                            <div className="text-left">
                                                <div
                                                    className={`font-pixel text-sm ${isPurchase ? "text-amber-400" : "text-stone-300"}`}
                                                >
                                                    Purchase this title
                                                </div>
                                                <div className="font-pixel text-[10px] text-stone-500">
                                                    Offer {title_type.purchase_cost}g (you have{" "}
                                                    {user_gold}g)
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <button
                                ...
                                disabled={
                                    !canSubmit || submitting || (isPurchase && !canAffordPurchase)
                                }
                            ...
                            {title_type.requires_ceremony && (
                                <p className="mt-3 font-pixel text-[10px] text-stone-500 text-center">
                                    Note: If approved, you must attend a ceremony to receive this
                                    title.
                                </p>
                            )}
                        </div>
