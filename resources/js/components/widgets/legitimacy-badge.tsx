...
export default function LegitimacyBadge({
    ...
    return (
        ...
        >
            ...
            <span className={`font-pixel ${sizes.value} ${config.color}`}>
                {legitimacy}%
            </span>
            {showLabel && (
                <span className={`font-pixel ${sizes.text} text-stone-500`}>
                    {config.label}
                </span>
            )}
            {showTrend === 'up' && (
                <TrendingUp className={`${sizes.icon} text-green-400`} />
            )}
            {showTrend === 'down' && (
                <TrendingDown className={`${sizes.icon} text-red-400`} />
            )}
        </div>
    );
}
        ...
        <div className={`rounded-lg border ${config.border} ${config.bg} p-3`}>
            <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                    <div className={`font-pixel text-xl ${config.color}`}>
                        {legitimacy}%
                    </div>
                    {recentChange !== undefined && recentChange !== 0 && (
                        <div className={`flex items-center justify-end gap-1 font-pixel text-[10px] ${
